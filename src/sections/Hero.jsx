                ...
                <div className='col-start-1 md:row-start-1'>
                    <h1 className='mb-8 text-[clamp(3rem,18vmin,20rem)] font-extrabold leading-none tracking-tighter' aria-label='Hi, Iam Aakash'>
                        <span className='block text-slate-300'>
                            <span>{renderLetters("Hi,","first")}</span> <span>{renderLetters("Iam","first")}</span>
                        </span>
                        <span className='-mt-[.2em] block text-slate-500'>{renderLetters("Aakash","last")}</span>
                        <span className='job-title  block bg-gradient-to-tr from-yellow-500 via-yellow-200 to-yellow-500 bg-clip-text text-2xl font-bold uppercase tracking-[.2em] text-transparent opacity-0 md:text-4xl'>Web Developer</span>
                    </h1>
